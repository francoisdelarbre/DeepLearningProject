...
from albumentations import Compose, RandomSizedCrop, OpticalDistortion, ElasticTransform, RandomRotate90, \
    ...
    RandomGamma, CLAHE, HueSaturationValue, ChannelShuffle
...
class DataGenerator(Sequence):
    def __init__(self, data_directory, output_masks, ids_list=None, batch_size=32,
                 resolution=128, n_channels_input=3, shuffle=True):
        ...
        self.augmentations = Compose([RandomSizedCrop(min_max_height=(int(resolution*2/3), int(resolution*3/2)),
                                                      height=resolution, width=resolution,
                                                      interpolation=cv2.INTER_NEAREST, p=1.0),
                                      OneOf([OpticalDistortion(p=1.0, distort_limit=2, shift_limit=0.5),
                                             GridDistortion(p=.5), ElasticTransform(p=1.0, alpha=120, sigma=120 * 0.05,
    ...
    def __getitem__(self, item):
        ...
        return self.__data_generation(ids_list_subset, data_augmentation=True)

    def get_unique_item(self, item):
        """same as item but only gives back a single element"""
        id_item = self.ids_list[self.indexes[item]]
        return self.__data_generation([id_item], data_augmentation=False)

    def __data_generation(self, ids_list_subset, data_augmentation):
        """ take a list of ids of images and return the corresponding batch to feed the network, this step includes data
        augmentation if data_augmentation is set to True, otherwise the image is rescaled"""
        ...
        if not data_augmentation:
            images = (cv2.resize(image, (self.resolution, self.resolution), interpolation=cv2.INTER_NEAREST)
                      for image in images)
        masks = (self.get_channels_masks(id_image=current_id, resize_masks=not data_augmentation)
    ...
    def get_channels_masks(self, id_image, resize_masks, processed_dir_name="processed_masks"):
        """ return the masks of an image. The image needs to have been processed and the compiled mask must be in the
        subdirectory named processed_dir_name, the masks are resized if resize_masks is set to true"""
        ...
        if resize_masks:
            mask_channels = (cv2.resize(mask_channel, (self.resolution, self.resolution),
                                        interpolation=cv2.INTER_NEAREST) for mask_channel in mask_channels)
